...
void main() {
  ...
  group('UserFlowBloc Tests', () {
    ...
    ///
    // blocTest<UserFlowBloc, UserFlowState>('emits states in order when GetAllCountriesNamesAndFlagsEvent is added',
    //     build: () {
    //       when(() => mockGetAllCountriesNamesAndFlags(MockNoParams())).thenAnswer(
    //         // (_) async => Right(tCountryNamesWithFlagsModel),
    //         (_) async => Right<UserFlowFailure, KtList<CountryNamesWithFlagsModel>>(tCountryNamesWithFlagsModel),
    //       );
    //       return userFlowBloc;
    //     },
    //     act: (bloc)=>
    //       bloc.add(const GetAllCountriesNamesAndFlagsEvent()) ,
    //     expect: () => [
    //           correctUserFLowState(userFlowBloc.state.copyWith(
